...
var fs = require("fs");
let rawdata = fs.readFileSync("players_full.json");
...
function buildPlayerInfoMessage(player) {
	...
	let player_data = fullPlayerData[player.id];
	...
	player_stats.forEach(stat => {
		...
		message+= "\n";
		message+= getScoreString(player_data[0][stat]) 
		message+= "\n";
	});
...
function getScoreString(score) {
	let score_string = score_icon.repeat(Math.floor(score/10)) + filler_icon.repeat(10 - Math.floor(score/10));
	return score_string;
}
