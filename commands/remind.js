	...
	async execute(message, args, client) {
		...
		if (!allowed_channels.includes(message.channel.id)) {
			return message.reply(
				"This command can only be sent in the #test-lab or #s7-talk channels"
			);
		}
		...
		if (args.length == 2) {
			let league = args[0];
			let game_num = args[1];
			let response_message = await message.channel.send(
			...
			response_message.edit(
				`Results checked: ${view_games.length * 2}\nResults missing: ${
					Object.keys(coaches_missing).length
