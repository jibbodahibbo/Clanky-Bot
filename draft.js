...
const draft2001Channel = "1305799066919112704"; //S15 c8
const draft2002Channel = "1305799099949256728"; //S15 c8
const draft2003Channel = "1305799124377141261"; //S15 c8
...
const draft_sheet_id = "11Ww_dEnFgXr5YmWG3VObkTluxd10xTcU4oesEvyLnnY"; //S15 c8 
...
async function getNextOpenPick(league) {
  ...
  let range = "A1:B252"; //updated to 252 to fit 28 teams in s15
  let result;
...
module.exports = {
	...
	async execute(message, args, client) {
      ...
      if (args.length==1 && args[0].length==2){
		...
		if (nextDrafter) {
			try {
				const user = await message.client.users.fetch(coach_data[nextDrafter]["id"]);
				await user.send(`You are On Deck to Draft in the rBBL ${draftData[league]["name"]} Draft. Get your pick ready.`);
			} catch (err) {
